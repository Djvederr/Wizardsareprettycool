...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, color, height, width,start_x,start_y):
        ...
        outputing("start : ", self.startRect.x, self.startRect.y)
...
def shoot():
    ...
    if player:
        if message2.spellno!=3:
            ...
            bullet = Bullet(COLORS[message2.spellno], 25, 25, start_x, start_y)
            bullets.add(bullet)
    else:
        if message1.spellno != 3:
            start_x, start_y = [gun1.rect.midright[0], gun1.rect.midright[1]]
            bullet = Bullet(COLORS[message1.spellno], 25, 25, start_x, start_y)
            bullets.add(bullet)
